...
# SYSTEM SIZE
nx=2 # Replicate system in x direction 
ny=1 # Replicate system in y direction 
nz=1 # Replicate system in z direction --> Total system size would be nx*ny*nz
...
# SLAB DIMENSIONS
boxsize=56.0 # The x and y dimensions of the slab configuration in nanometers
slab_z_length = 56.0 # The z dimension of the slab configuration in nanometers

# RESIZING RUN PARAMETERS
resize_T=300 # Temperature for resizing run in Kelvin
resize_steps= 500000 #500000 # Number of steps (>500) to be used for resizing the box to boxsize set previously
if resize_steps<500:
    resize_steps=501
...
production_steps=1000000  #100000000 # Total number of steps 
...
hoomd.dump.gsd('resize.gsd', period=1000, group=all, truncate=True)
#hoomd.dump.dcd('resize.dcd', period=100, group=all, overwrite=True)
...
integrator = hoomd.md.integrate.langevin(group=all, kT=temp, seed=seed) # Temp is kT/0.00831446
for cnt,i in enumerate(aakeys):
    integrator.set_gamma(i,gamma=aamass[cnt]/1000.0)
## Outputs
...
hoomd.dump.dcd(fileroot+'_dump_{}.dcd'.format(seed), period=100000, group=all, overwrite=False)
